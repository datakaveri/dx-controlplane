...
public class UserAccessHandler implements Handler<RoutingContext> {
  ...
  public UserAccessHandler(
      PostgresService postgresService, KeycloakUserService keycloakUserService) {
  ...
  public void handle(RoutingContext event) {
    ...
    keycloakUserService
        .getUserById(userId)
        .compose(dxUser -> insertUserIntoDb(dxUser, event))
        .onSuccess(
            result -> {
  ...
  private Future<QueryResult> insertUserIntoDb(DxUser dxUser, RoutingContext ctx) {
    LOGGER.debug("inside insert user in DB method");
    RoutingContextHelper.setDxUser(ctx, dxUser);
    ...
    return postgresService
        ...
        .onSuccess(
            result ->
                LOGGER.debug(
                    "User with ID {} inserted successfully: {}", dxUser.sub(), result.toString()))
        .onFailure(
            err -> {
              LOGGER.error(
                  "Failure while executing user insertion query: {}", err.getMessage(), err);
              throw new DxRuntimeException(HttpStatusCode.getByValue(500).getValue(), "db_error");
            });
  }
