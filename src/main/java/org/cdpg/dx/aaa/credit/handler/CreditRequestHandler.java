...
public class CreditRequestHandler {
  ...
  public void createCreditRequest(RoutingContext ctx) {
    ...
    JsonObject creditRequestJson =
        Optional.ofNullable(ctx.body().asJsonObject()).orElse(new JsonObject());
                      ...
                      enrichedList -> {
                        UserActivityAuditLogBuilder auditLogBuilder =
                            CreditRequestAuditLogHelper.buildAudit(
                                ctx,
                                new JsonObject(),
                                CreditRequestAuditOperation.GET_CREDIT_REQUESTS);
                        CpRoutingContextHelper.setAuditingLogV2(ctx, auditLogBuilder);
                        ResponseBuilder.sendSuccess(
                            ctx, enrichedList, result.paginationInfo(), this.urnGenerator);
                      })
                  .onFailure(ctx::fail);
            })
        .onFailure(ctx::fail);
  ...
  public void getUserCreditRequests(RoutingContext ctx) {
    ...
    creditService
        .getCreditRequestsByUserId(userId)
        .onSuccess(
            creditRequests -> {
        ...
        .onFailure(
            err -> {
              LOGGER.error(
                  "Failed to fetch credit requests for user {}: {}", userId, err.getMessage());
              ctx.fail(err);
            });
  }
  ...
  public void updateCreditRequestStatus(RoutingContext ctx) {
    ...
    if (status == GRANTED) {
      ...
      try {
        ...
        throw new DxBadRequestException(
            "Invalid expiration date format. Expected format: " + FORMATTER);
      }
    ...
    creditService
        .updateCreditRequestStatus(requestId, status, transactedBy, amount, expirationDate)
        .onSuccess(
            transaction -> {
              //        AuditLog auditLog = AuditingHelper.createAuditLog(ctx.user(),
              //          RoutingContextHelper.getRequestPath(ctx), "PUT", "Credit Request Status
              // Updated");
              //        RoutingContextHelper.setAuditingLog(ctx, auditLog);

              JsonObject auditData =
                  transaction != null
                      ? transaction.toJson()
                      : new JsonObject().put("id", requestId.toString());
              UserActivityAuditLogBuilder auditLogBuilder =
                  CreditRequestAuditLogHelper.buildAudit(
                      ctx, auditData, CreditRequestAuditOperation.UPDATE);
              CpRoutingContextHelper.setAuditingLogV2(ctx, auditLogBuilder);
              ...
              if (transaction != null) {
                ...
                ResponseBuilder.sendSuccess(
                    ctx, "Credit request rejected successfully", this.urnGenerator);
              }
  ...
  public void deletePendingCreditRequest(RoutingContext ctx) {
    ...
    creditService
        .getCreditRequestById(requestId)
        .compose(
            request -> {
              if (request == null) {
              ...
              if (!request.userId().equals(userId)) {
                return Future.failedFuture(
                    new DxForbiddenException(
                        "User is not authorized to delete this credit request"));
              }

              if (!request.status().equals(Status.PENDING.getStatus())) {
                return Future.failedFuture(
                    new DxBadRequestException("Only pending credit requests can be deleted"));
              }
        ...
        .onSuccess(
            deleted -> {
              ...
              UserActivityAuditLogBuilder auditLogBuilder =
                  CreditRequestAuditLogHelper.buildAudit(
                      ctx,
                      new JsonObject().put(ID, requestIdStr),
                      CreditRequestAuditOperation.DELETE);

              CpRoutingContextHelper.setAuditingLogV2(ctx, auditLogBuilder);

              ResponseBuilder.sendSuccess(
                  ctx, "Pending Credit Request deleted successfully", urnGenerator);
            })
        .onFailure(ctx::fail);
  }
}
