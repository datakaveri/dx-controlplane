...
public class ResourceServerController implements ApiController {
  ...
  private final URNGenerator urnGenerator;
    ...
    Handler<RoutingContext> orgAndCosAdminAccessHandler = AuthorizationHandler.forRoles(DxRole.ORG_ADMIN,DxRole.COS_ADMIN);

    public ResourceServerController(ResourceServerService service, AuditingHandler auditingHandler, URNGenerator urnGenerator) {
  ...
  public void register(RouterBuilder builder) {
    builder.operation(ApiConstants.CREATE_RESOURCE_SERVER)
        .handler(auditingHandler::handleApiAudit)
        .handler(orgAndCosAdminAccessHandler)
        .handler(this::handleCreate);

    builder.operation(ApiConstants.LIST_RESOURCE_SERVERS)
        .handler(auditingHandler::handleApiAudit)
        .handler(orgAndCosAdminAccessHandler)
        .handler(this::handleList);

    builder.operation(ApiConstants.GET_RESOURCE_SERVER)
      .handler(auditingHandler::handleApiAudit)
            .handler(orgAndCosAdminAccessHandler)
...
//            .handler(this::handleUpdate);
  ...
  private void handleCreate(RoutingContext ctx) {
    ...
    service.create(rs)
      ...
      .onFailure(err -> ctx.response().setStatusCode(400).end(new JsonObject().put("error", err.getMessage()).encode()));
  }
  ...
  private void handleList(RoutingContext ctx) {
    ...
    service.getAllByRole(userRole, userId)
        .onSuccess(resourceServers -> {
  ...
  private void handleGet(RoutingContext ctx) {
    ...
    service.get(UUID.fromString(id))
  ...
  private void handleUpdate(RoutingContext ctx) {
    ...
    service.update(UUID.fromString(id), updates)
  ...
  private void handleDelete(RoutingContext ctx) {
    ...
    service.deleteByRole(UUID.fromString(id), userRole, userId)
