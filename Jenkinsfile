    ...
    stage('Continuous Deployment') {
      ...
      stages {
        ...
        stage('Docker Swarm deployment') {
          steps {
            script {
              sh "ssh azureuser@docker-swarm 'docker service update tgdex-controlplane_controlplane-tgdex --image ghcr.io/datakaveri/controlplane-dev:tgdex-1.0.0-${env.GIT_HASH}'"
              sh 'sleep 15'
              sh '''#!/bin/bash 
